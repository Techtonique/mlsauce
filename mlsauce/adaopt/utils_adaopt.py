...
def update_proba(probs, misclass, w_new, w_prev, v_prev, eta, gamma, n_obs, n_classes):
...
def fit_adaopt(X, y, n_iterations, n_classes, learning_rate, reg_lambda, reg_alpha,
               eta, gamma, tolerance):
    ...
    for m in range(n_iterations):
        ...
        err_m += reg_lambda * (reg_alpha * np.sum(np.abs(w_prev)) + (1 - reg_alpha) * 0.5 * np.sum(np.power(w_prev, 2)))
        ...
        probs, v_prev = update_proba(probs, misclass, w_new, w_prev, v_prev, eta, gamma, len(X), n_classes)
        preds = np.argmax(probs, axis=1)
...
def predict_proba_adaopt(X_test, scaled_X_train, probs_train, k, n_clusters, seed,
                         batch_size=100, type_dist="euclidean", cache=True):
    ...
    if n_clusters > 0:
        ...
        kmeans = MiniBatchKMeans(n_clusters=n_clusters, batch_size=batch_size, random_state=seed).fit(scaled_X_train)
    ...
    dist_mat = np.sum(X_test ** 2, axis=1)[:, None] + np.sum(scaled_X_train ** 2, axis=1) - 2 * np.dot(X_test, scaled_X_train.T)

    for i in range(n_test):
        dists_test_i = dist_mat[i]
        kmin_test_i = np.argsort(dists_test_i)[:k]
        weights_test_i = 1 / np.maximum(dists_test_i[kmin_test_i], np.finfo(float).eps)
...
def predict_adaopt(X_test, scaled_X_train, probs_train, k, n_clusters, seed,
                   batch_size=100, type_dist="euclidean", cache=True):
    return np.argmax(predict_proba_adaopt(X_test, scaled_X_train, probs_train, k, n_clusters, seed,
                                          batch_size, type_dist, cache), axis=1)
