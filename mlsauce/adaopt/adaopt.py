...
class AdaOpt():    
    ...
    def fit(self, X, y, **kwargs):    
        ...
        assert (n == len(y)),\
        "must have X.shape[0] == len(y)"        
        
        res = ao.fit(X=X, y=y, 
                     n_iterations=self.n_iterations,
                     n_X=n, p_X=p, n_classes=n_classes,
    ...
    def predict(self, X, type_dist="euclidean", 
                **kwargs):
        """Predict test data X.
        ...
        **kwargs: additional parameters to be passed to 
        ...
        return np.argmax(
            self.predict_proba(X, type_dist=type_dist, 
                               **kwargs), axis=1
        )
            

    def predict_proba(self, X, type_dist="euclidean", 
                      **kwargs):
