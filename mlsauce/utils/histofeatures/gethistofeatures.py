...
def create_histogram_with_bin_values(x, bins="auto"):
    """
    ...
    return {
        i: (bin_edges[i], bin_edges[i + 1]) for i in range(len(bin_edges) - 1)
    }
...
def get_histo_features(X, bin_value_dict=None, bins="auto"):
    """
    ...
    if bin_value_dict is None:  # means training set case

        if isinstance(X, pd.DataFrame):
            colnames = X.columns
            X = X.values
            X_hist = pd.DataFrame(np.zeros(X.shape), columns=colnames)
        ...
        if isinstance(X, pd.DataFrame):
            ...
            X_hist = pd.DataFrame(np.zeros(X.shape), columns=colnames)
            for i, col in enumerate(colnames):
                X_hist[col] = assign_values_to_input(X[:, i], bin_value_dict=bin_value_dict)
        else:
            X_hist = np.zeros(X.shape)
            for i in range(X.shape[1]):
                X_hist[:, i] = assign_values_to_input(X[:, i], bin_value_dict=bin_value_dict)
