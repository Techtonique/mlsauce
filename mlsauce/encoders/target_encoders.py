...
def corrtarget_encoder(df, target, rho=0.4, verbose=1, seed=123):
    ...
    for col in col_iterator:
        ...
        if (X_dtypes[col] == np.object): # something like a character string
            ...
            X_temp = qr.summarize(df_, req = col + ', avg(pseudo_target)', 
                                   group_by = col)
            ...
            for l in levels:
                ...
                X_numeric = qr.setwhere(X_numeric, col=col, val=l, 
                            replace=val, copy=False)
    ...
    return X_numeric, np.corrcoef(df[target].values, df_['pseudo_target'].values)[0, 1]